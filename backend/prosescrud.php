...
class prosesCrud {
    ...
    function tampil_artikel($tabel)
    {
        $row = $this->db->prepare("SELECT * FROM $tabel");
        $row->execute();
        return $hasil_artikel = $row->fetchAll();
    }
    ...
    function tampil_last_id($tabel,$where,$id)
    {
        $row = $this->db->prepare("SELECT * FROM $tabel WHERE $where ORDER BY $id = ? DESC LIMIT 1");
    ...
    function tampil_data_artikel($tabel,$where,$id)
    {
        $row = $this->db->prepare("SELECT * FROM $tabel WHERE $where = ?");
        $row->execute(array($id));
        return $hasil = $row->fetch();
